...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT path FROM documentacao_contrato WHERE iddocumentacao_contrato = $id_documento";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $sql_delete = "DELETE FROM documentacao_contrato WHERE iddocumentacao_contrato = $id_documento";
        if (mysqli_query($conn, $sql_delete)) {
            // Remove o arquivo do sistema de arquivos
            if (file_exists($caminho_arquivo)) {
                unlink($caminho_arquivo); // Deleta o arquivo
            }
            header("Location: detalhes_contrato_aluguel.php?id=" . $_GET['id_contrato']);
            exit;
        } else {
